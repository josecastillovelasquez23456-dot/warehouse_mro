...
def upload_inventory():
    ...
    Sobrescribe InventoryItem y guarda snapshot en InventoryHistory.
    """
...
def upload_inventory_history():
    """
    Sube inventarios antiguos SOLO a InventoryHistory.
    No toca el inventario actual.
    """
    if request.method == "POST":
        file = request.files.get("file")
        snapshot_name = request.form.get("snapshot_name", "").strip()
...
def discrepancies():
    """
    Sube un Excel de conteo físico y genera Excel de discrepancias.
    """
    if request.method == "POST":
        file = request.files.get("file")
        if not file:
